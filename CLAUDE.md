...
The Recipe block carries `datePublished`, `dateModified` and
`totalTime`, and the sitemap carries `lastmod`, both off
`data/dates.json`. That file is generated by `make pages` and keyed on a
hash of each drink object: a new drink is published the day the script
runs, a changed one is modified that day, and `make verify` refuses an
entry whose hash no longer matches the drink. It was seeded once from
the git history of `cocktails.json`; do not edit it by hand, and do not
make the check ask git, because CI checks out one commit.

Every row on the Menu links its name to that page, so the front page
links to all of them. A link cannot live inside a button, so the row is
a `div` holding the expand button, laid over the whole row, and the
text beside it with the name as the link above that button. A plain
click on the name still expands the row; a modifier click is the
browser's. The text is the button's name through `aria-labelledby` and
is `aria-hidden` as text, and the link is out of the tab order, so a
screen reader and a keyboard meet one control per row as before. The
button comes first in the row, because focus and the aside's Close look
the row up by `[data-drink]`.
...
- **The origin carries what the site serves, not the repo.**
  ...
  name. `make probe` reads the live origin: http goes to https, HSTS
  and the security headers are there, and the CSP names those hashes.
  It is read-only and never part of `make verify`, because CI has no
  business depending on the live edge.
